# Step-by-Step guide to a LUCKYBIT Masternode setup
...
If you haven't done so already, install the windows wallet from https://github.com/luckybitcoin/luckycore/releases 
...
Then send 1000 LUCKY coins to the address you just created. Make sure the address receives EXACTLY 1000 coins, so DO NOT tick the "Subtract fee from amount" option.
...
```
wget -O slucky.sh https://git.io/vh0Nr && sh slucky.sh
```
...
The script will create a new system user named **luckyuser** and will generate a random password.
...
```
MN_NAME MN_PUBLIC_IP:7717 MN_KEY TX_ID VOUT
```
...
**LUCKY**: LZguMLmiNyAdUw64ipH5DPuXMtojzCyv53 
